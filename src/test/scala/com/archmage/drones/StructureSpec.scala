...
class StructureSpec extends FlatSpec {
  ...
  "A structure with scrap that gets gathered" should "decrease its scrap by 1 each time a gather occurs" in {
    ...
    val structure = Structure(Geo(0, 0), Scrapheap(), startingScrap)
    var world = World(Map(1 -> Drone(Geo(), State[DroneState](Gather()))), Set(structure))
  ...
  "A structure that has scrap deposited in it" should "increase its scrap count accordingly" in {
    val startingScrap = 5
    val world = World(Map(1 -> Drone(Geo(), State[DroneState](Deposit()), Queue(), startingScrap)),
      Set(Structure()))
